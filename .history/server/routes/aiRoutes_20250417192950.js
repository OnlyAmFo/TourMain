...
router.post('/chat', async (req, res) => {
  try {
    ...
    const prompt = `<system>You are a helpful travel assistant. Your role is to provide clear, concise travel advice and answer questions about destinations, planning, and travel tips. Keep responses focused on travel-related topics and use a friendly, professional tone.</system>
...
<assistant>`;
    ...
    try {
      ...
      let formattedResponse = response.data[0].generated_text;
      
      // Remove system prompts and XML-like tags
      formattedResponse = formattedResponse.replace(/<system>.*?<\/system>/gs, '');
      formattedResponse = formattedResponse.replace(/<user>.*?<\/user>/gs, '');
      formattedResponse = formattedResponse.replace(/<assistant>|<\/assistant>/g, '');
      
      // Remove any remaining HTML-like tags
      formattedResponse = formattedResponse.replace(/<[^>]*>/g, '');
      
      // Remove any email addresses
      formattedResponse = formattedResponse.replace(/[\w\.-]+@[\w\.-]+\.\w+/g, '');
